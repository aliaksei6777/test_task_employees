...
test('users should be set', () => {
    ...
    const endState = reducer({users: [],isFetching: false}, setEmployees(startState.users))
...
test('correct user should be added', () => {
    ...
    const endState = reducer(startState, addEmployee(newUser))
...
test('correct user should be deleted', () => {
    const userId = 4
    const endState = reducer(startState, removeEmployee(userId))
...
test('fetching status should be changed', () => {
    ...
    const endState = reducer(startState, toggleIsFetching(true))
